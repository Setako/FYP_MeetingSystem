...
export class MeetingStateHolderService {
    ...
    constructor(ipc: IPCService, readonly windowStackService: WindowStackService, readonly snackBar: MatSnackBar) {
        ipc.on('show-token', (event, data: { accessToken: string }) => this.showToken(data.accessToken));

        ipc.on('take-over', (event, data: { controlToken: string; meeting: any }) => this.takeOver(data.meeting));
        ipc.on('server-disconnected', () => this.disconnected());

        ipc.on('server-exception', (data: any) => {
            snackBar.open('Server exception catched', 'Dismiss', {duration: 4000});
