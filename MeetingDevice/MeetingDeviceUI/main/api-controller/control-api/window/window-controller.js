...
router.post('/resize', async function (req, res) {
    ...
    if (req.body.resizeSessionIdentifier !== lastResizeSessionIdentifier) {
        ...
        originalWindowSize = [
            parseInt(currentWindowInfo['Width']),
            parseInt(currentWindowInfo['Height']),
        ];
    }
...
router.post('/drag', async function (req, res) {
    ...
    if (req.body.dragSessionIdentifier !== lastDragSessionIdentifier) {
        ...
        originalWindowLocation = [
            parseInt(currentWindowInfo['Absolute upper-left X']),
            parseInt(currentWindowInfo['Absolute upper-left Y']),
        ];
    }
    ...
    promiseExec(
        `xdotool windowmove ${currentWindowInfo['Window id']} ${newX} ${newY}`,
    ).catch(e => console.log(e));
